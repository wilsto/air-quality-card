...
`;const Te=.1;const Ne=3;const je={direction:null,count:0};function computeTrend(e,s){if(e==null||!Number.isFinite(e))return je;const o=s!=null&&Number.isFinite(s)&&s>0?s:Te;const l=Math.floor(Math.abs(e)/o);if(l<1)return je;return{direction:e>0?"rising":"falling",count:Math.min(Ne,l)}}function trendGlyphs(e){if(!e||!e.direction||e.count<1)return"";return(e.direction==="rising"?"▴":"▾").repeat(e.count)}function trendLabelKey(e){if(!e||!e.direction||e.count<1)return null;return`trend.${e.direction}_${Math.min(Ne,e.count)}`}const labelShift=e=>{if(e<=.5)return"translateX(0)";if(e>=99.5)return"translateX(-100%)";return"translateX(-50%)"};const markerShift=e=>{const s=Math.max(0,Math.min(100,e));const o=Number(s.toFixed(2));const l=Number((100-s).toFixed(2));return`translateX(clamp(${-o}cqw, -50%, calc(${l}cqw - 100%)))`};class cardContent{static generateTitle(e){const s=e.title!==void 0?oe` <h1 class="pool-monitor-title">${e.title}</h1> `:oe``;return oe`${s}`}static generateStatusBadge(e,s){return oe`
    ...
    `}static generateTrend(e){const s=trendGlyphs(e.trend);if(!s)return"";return oe`<span class="trend-arrow" aria-hidden="true">${s}</span
      ><span class="sr-only">${e.trend_label||""}</span>`}static generateBody(e,s){if(!s)return oe` <div class="warning-message">No sensor data available</div> `;const o=s.pct_marker;const l=markerShift(o);const u=o<=1?"translateX(0)":o>=99?"translateX(-100%)":"translateX(-50%)";return oe`
      <!-- ##### ${s.name} section ##### -->
      <div
...
`;const editorText=(e,s)=>getTranslation((e===null||e===void 0?void 0:e.language)||"en",`editor.${s}`);class MonitorEditorBase extends i{t(e){return editorText(this.hass,e)}setConfig(e){this._config=Object.assign(Object.assign({},e),{display:Object.assign(Object.assign({},ze),e.display),colors:Object.assign(Object.assign({},Fe),e.colors),sensors:e.sensors||{}})}render(){if(!this.hass||!this._config)return oe``;return oe`
    ...
    `}_renderAddSensor(){if(this.freeform)return oe`
      ...
      `;const e=Object.entries(this.registry).filter(([e])=>!this.sensors[e]).map(([e,s])=>({value:e,label:s.name,category:s.category||"other"}));if(e.length===0)return oe`<div class="empty-message">${this.t("all_configured")}</div>`;const s=e.reduce((e,s)=>{const o=s.category;if(!e[o])e[o]=[];e[o].push(s);return e},{});const o=[...new Set(Object.values(this.registry).map(e=>e.category||"other"))];const l=[...o.filter(e=>e!=="other"),...o.filter(e=>e==="other")];return oe`
